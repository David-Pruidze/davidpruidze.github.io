...
const cvx = (window.innerWidth - viewportWidth) * .5;
const cvy = (window.innerHeight - viewportHeight) * .5;
// Объявляем DOM-элементы
let progress;
let menu;
let recordsMenu;
let recordsButton;
let storeUserNameButton;
...
let mouseX = (window.innerWidth - viewportWidth);
let mouseY = (window.innerHeight - viewportHeight);
...
gameFunc = new function () {
    ...
    function deviceorientationHandler(event) {
        mouseX += event.gamma * 0.8;
        mouseY += event.beta * 0.8;
    }

    // Фунция обработчик события изменения размера
    function resizeHandler() {
        let margin;
        const desiredMenuWidth = 500;
        // Определяем игровое поле, присваиваем и позиционируем CANVAS в соответствии с новыми размерами
    ...
    // Функция для воспроизведения виброотклика на соответствующие события в игре
    ...
    this.startGame = function () {
        ...
        if (playing === false) {
            ...
            if (playing === true) {
                const newElements = document.getElementsByClassName("data");
...
//уровни
function levelUp() {
    const adjust = timer / 45;
    return adjust
}
//формируем лист фигур
function createCircle(circleList) {
    const circle = new Circle(randomInt(5, 12));
...
//функция удаления элементов из листа
