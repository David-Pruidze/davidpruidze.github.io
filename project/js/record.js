let records = new function () {
    ...
    function addRecordToTable() {
        ...
        const existingUserScore = recordStorage.filter(function (result) {
            return userName === result.name;
        });
        userScoreExists = existingUserScore.length > 0;

        // (если нашли) проверяем, набрал ли он больше очков, чем в прошлый раз
        if (userScoreExists) {
